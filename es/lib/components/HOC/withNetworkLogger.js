...
const withNetworkLogger = WrappedComponent => {
  const NetworkLogger = props => {
    ...
    const handleNetworkResponse = useCallback(async response => {
      let request = null;
      if (response instanceof Response) {
        ...
        request = Array.from(requests.keys()).find(r => {
          ...
          return r.url === response.url && r.method === ((_response$request = response.request) == null ? void 0 : _response$request.method);
        });
      }
      ...
      if (request) {
        ...
        const responseData = {
          ...
          headers: response ? [...response.headers].reduce((acc, curr) => _extends({}, acc, {
            [curr[0]]: curr[1]
          }), {}) : null
        };
